...
class encode {
	...
	public static function encrypt($txt, $key = 'xxe1e1e1e1e1ffff') {
		$s = urlencode(strtr(base64_encode(self::xxtea_encrypt($txt, $key)), '+/', '-_'));
		return $s;
	}
	
	public static function decrypt($txt, $key = 'xxe1e1e1e1e1ffff') {
		return self::xxtea_decrypt(base64_decode(strtr(urldecode($txt), '-_', '+/')), $key);	
	}
}
